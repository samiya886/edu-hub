  ...
  return (
    <section className="py-24 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
          ...
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:flex-wrap sm:gap-4">
            {features.map((f, i) => (
              <div
                key={i}
                className="flex w-full min-w-0 items-center justify-center gap-2 rounded-full border border-gray-100 bg-gray-50 px-4 py-2 text-center text-sm font-bold text-gray-500 sm:w-auto sm:justify-start sm:text-left"
              >
                <span className="shrink-0 text-[#0a4a44]">{f.icon}</span>
                <span className="whitespace-normal leading-snug sm:whitespace-nowrap">{f.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Services Grid */}
        <div className="mobile-carousel mobile-scroll-track md:grid-cols-3 md:gap-8">
