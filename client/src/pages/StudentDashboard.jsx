    ...
    <main className={`${sidebarOpen ? 'lg:ml-[280px]' : 'lg:ml-[96px]'} min-w-0 transition-[margin] duration-300`}>
      <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-gray-100 bg-white/80 px-4 backdrop-blur-2xl md:px-8 lg:h-24 lg:px-10">
        <div className="flex min-w-0 items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-[0.25em] text-gray-400">Student Workspace</p>
    ...
    <form onSubmit={onSubmit} className="space-y-6 p-6 md:p-10">
      ...
      <Field label="Description">
        <textarea
          ...
          rows="4"
          placeholder="Short context for this resource"
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
        ...
        {editingId && (
          <button
            type="button"
            onClick={onCancel}
            className="rounded-3xl bg-gray-50 px-8 py-5 text-sm font-black text-[#0a4a44] transition hover:bg-gray-100"
          >
            Cancel
          </button>
        )}
      ...
      {message && (
        <div className="rounded-2xl bg-[#0a4a44]/5 p-4 text-sm font-bold text-[#0a4a44]">{message}</div>
      )}
    </form>
  </motion.div>
);
